# Code by Jakub Grzesik (www.kivano.net)
...
func _ready():
	...
	parent = get_parent();
	halfSize = bg.get_item_rect().size*0.5;
...
func need2ChangeActivePointer(event): #touch down inside analog
	...
	if mouseButton or touch:
		if isDynamicallyShowing:
			...
			return lenght < (squaredHalfSizeLenght*2)
	else:
	 return false
...
func sendSignal2Listener():
	if (listenerNodePath != null):
		listenerNodePath.analog_force_change(currentForce)
